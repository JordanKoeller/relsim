...
export function NewPrefabCache(gl: GL): LRUCache<{shader: Shader, url: string}, Promise<Prefab | null>> {
  return NewLRUCache(
    200,
    (elem) => elem.url,
...
export async function LoadPrefab(gl: GL, shader: Shader, url: string): Promise<Prefab | null> {
...
export function ClearCanvas(glContext: GLContext) {
  ...
  glContext.gl.clear(glContext.gl.COLOR_BUFFER_BIT | glContext.gl.DEPTH_BUFFER_BIT);
...
export function CreateStaticBuffer(gl: GL, shader: Shader, buffer: BufferSpec): Buffer | null {
  // Create VAO.
  // @ts-ignore
  ...
  let offset = 0;
  for (let i = 0; i < buffer.attributes.length; i++) {
    const attributeId = gl.getAttribLocation(shader.id, buffer.attributes[i].name);
...
// Returns:
//   A texture object representing the image loaded to the GPU.
export async function CreateTextureFromUrl(gl: GL, url: string): Promise<Texture> {
...
//  
export async function LoadUniform(gl: GL, uniform: UniformSpec, url: string): Promise<Uniform | null> {
...
export function BindUniform(glContext: GLContext, shader: Shader, uniform: Uniform) {
